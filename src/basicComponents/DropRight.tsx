...
export const Dropright = ({
    ...
    function toggleOnHover(activate: boolean) {
        if (!isShowFixated) {
            setShowDropDown(activate);
        }
    }
    ...
    if (items.length !== 0)
        return (
            <div style={dropdownStyle}
                ref={ref}
            >
                <div className='row' onMouseEnter={() => { toggleOnHover(true) }} onMouseLeave={() => { toggleOnHover(false) }}>
                    <Button
                        key={-1}
                        content={content}
                        state={state}
                    ...
                    <div style={getContentStyle(showDropDown, ref)}>
                        {items}
                    </div>
                </div>
            </div >
...
function getContentStyle(currentState: Boolean, ref: React.RefObject<HTMLDivElement>): React.CSSProperties {
